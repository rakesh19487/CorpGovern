...
function initGame() {
	...
	quesbank=[];
    if(flag===0) {
        quesbank=Question.getAllByWeight(4, 0);
        shuffle(quesbank);
    }
    else {
        for(var i=flag; i<5; i++) {
            if(i==flag) {
                quesbank.push(Question.getBySubCat(4, question.category));
            }
            else
                quesbank.unshift(Question.getByWeightExSubcat(4,question.category));
        }
    }
}
...
function endGame(message, question) {
    ...
    if (message == "Great! Go ahead and meet the Yeti. He knows the secret way to the top!") {
        ...
        $(".btn-ok").css({
            top: "62%",
            left: "57%",
            "background": "url(../../img/active_instructionbuttonicy.png)",
